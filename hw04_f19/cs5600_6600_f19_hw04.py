...
def plot_costs(eval_costs, train_costs, num_epochs):
    plt.plot(num_epochs, eval_costs,'g', num_epochs, train_costs, 'b')
...
def plot_accuracies(eval_accs, train_accs, num_epochs):
    plt.plot(num_epochs, eval_accs,'g', num_epochs, train_accs, 'b')
    plt.title('Evaluation Acc(green) and Training Acc(blue)')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    plt.show()
...
def collect_1_hidden_layer_net_stats(lower_num_hidden_nodes,
                                     ...
                                     train_data,
                                     eval_data):
    # build a dictionary of range(upper_num_hidden_nodes, lower_num_hidden_nodes) elements with each having the results of net.SGD in their values for training a network that matches that value
    # We don't have to do all the computations we can quit when it stops improving
    hidden_layer_net_stats = {}
    for i in range(lower_num_hidden_nodes, upper_num_hidden_nodes + 1):
        net = network2.Network([784,i,10], cost=cost_function)
        ...
        hidden_layer_net_stats[i] = net_stats
